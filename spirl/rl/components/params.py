...
def get_args():
    ...
    # human in the loop
    parser.add_argument('--learn_reward', default=False, type=bool,
                        help='use human in the loop to train a reward model')
    parser.add_argument('--high_level', default=True, type=int, help='If False, learn pebble over atomic transitions')
    parser.add_argument('--task', default='mkb', type=str,
                        help='Default: microwave, kettle, bottom burner [, hinge cabinet (c)]')
    parser.add_argument('--reward_batch', default=128, type=int, 
                        help='Number of human queries provided at one feedback session')
    parser.add_argument('--pretrain_sac_loc', default="exp/pretrained_sac", type=str,
                        help='location of pretrained SAC agent')
